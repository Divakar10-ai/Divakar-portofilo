...
async function main() {
  ...
  if (!app.stage.ok) {
    return degrade('WebGL unavailable');
  }
  ...
  try {
    manifest = await fetch(
      `${MEDIA}manifest.json?v=${ASSET_VERSION}`
    ).then((r) => r.json());
  } catch {
  ...
  const tick = () => {
    bootFill.style.width = `${(++done / steps) * 100}%`;
  };
  ...
  const mk = (name, loopFade) => {
    ...
    return new Clip({
      src: [
        {
          url: `${MEDIA}${c.webm}?v=${ASSET_VERSION}`,
          type: 'video/webm',
        },
        {
          url: `${MEDIA}${c.mp4}?v=${ASSET_VERSION}`,
          type: 'video/mp4',
        },
      ],
...
function begin() {
  ...
  if (reduced) {
    // Honour reduced motion fully: land on the finished composition and hold it
...
function layout() {
  ...
  if (
    !app.word ||
    Math.abs(
      app.word.capPx - capPx
    ) > 2
  ) {
    const word =
      buildWord(
        capPx,
        app.stage.maxTexture
      );
...
function frame(now) {
  ...
  app.stage.parallax.x =
    p.x * gate;

  app.stage.parallax.y =
    p.y * gate;

  const state =
    sample(
      t,
      app.word.letters.length,
      app.order
    );
...
window.__shot = async (
  ...
  const url =
    app.stage.canvas.toDataURL(
      'image/png'
    );

  await fetch(
    `/__shot?name=${encodeURIComponent(
      name
    )}`,
    {
      method: 'POST',
  ...
  return `${app.stage.canvas.width}x${app.stage.canvas.height} @ t=${t.toFixed(
    2
  )}`;
};
